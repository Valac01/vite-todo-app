<template>
  <div class="mt-2">
    ...
    <div v-if="isTaskAvailable">
      <task-item v-for="task in tasks" :key="task" :task="task" @task-edit="taskEdit" @task-complete="taskComplete" @task-delete="taskDelete(task)"/>
    </div>
    <div v-else class="mt-6 flex flex-col-reverse">
      <h3 class="text-gray-400 text-center">There is no task available, you can sleep for now.</h3>
